...
CSV_FILE_PATH = "C:\Pablo\PythonCourse\CA1CSys-Des\data\pricelist_sten.csv"
...
def insert_values_to_table(table_name, csv_file):
    ...
    if conn is not None:
        ...
        names_price = ["codigo_lista", "inicio_vigen", "fin_vigen"]
        dict_price_lst = {"codigo_lista": int, "inicio_vigen": str, "fin_vige": str}

        df = pd.read_csv(csv_file, delimiter=";", decimal= ",", encoding="utf-8", skiprows=1, header=None, names=names_price, dtype=dict_price_lst)
        df["inicio_vigen"] = pd.to_datetime(df["inicio_vigen"], format="%d-%m-%Y")
        df["fin_vigen"] = pd.to_datetime(df["fin_vigen"], format="%d-%m-%Y")
...
if __name__ == "__main__":
    insert_values_to_table("pricelist_sten", CSV_FILE_PATH)
